...
// Append SVG für Balkendiagramm der Motorentypen 
var svg = d3.select("#balkenmotors")
.append("svg")
  .attr("width", width + margin.left + margin.right)
  .attr("height", height + margin.top + margin.bottom)
.append("g")
...
function drawBarChart(data) {
  ...
  slider.oninput = function() {
      updateBarChart(data, this.value);
  };
}
...
function updateBarChart(data, year) {
  ...
  var x = d3.scaleBand()
      .range([0, width])
      .domain(motorCounts.map(function(d) { return d.key; }))
      .padding(0.2);

  var y = d3.scaleLinear()
      .domain([0, d3.max(motorCounts, function(d) { return d.value; })])
      .range([height, 0]);
  ...
  svgBar.selectAll("g.axis").remove();
  svgBar.append("g")
